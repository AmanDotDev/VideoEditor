...
export const Menu = observer(() => {
  ...
  return (
    <ul className="bg-zinc-900 h-full">
      {MENU_OPTIONS.map((option) => {
        const isSelected = store.selectedMenuOption === option.name;
        return (
          <li
            key={option.name}
            className={`h-[50px] flex flex-col items-center justify-center ${isSelected ? "bg-zinc-600" : ""}`}
          >
            <button
            ...
            >
              ...
              />
              <div
                className={`text-[0.6rem] hover:text-zinc-400 ${isSelected ? "text-white" : "text-zinc-300"}`}
              >
                {option.name}
              </div>
            </button>
